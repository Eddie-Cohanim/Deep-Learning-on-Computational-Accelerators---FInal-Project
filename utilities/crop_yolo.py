...
def main() -> None:
    argument_parser = argparse.ArgumentParser(
        description="Crop YOLO detections from images and save them by class."
    )
    ...
    argument_parser.add_argument(
        ...
        help="Path to the YOLO detections JSON file.",
    )
    argument_parser.add_argument(
        "--confidence-threshold",
        type=float,
        default=0.25,
        help="Minimum confidence score to include a detection (default: 0.25).",
    )
    parsed_args = argument_parser.parse_args()

    output_folder = crop_and_save_by_class(
        input_folder=parsed_args.input_folder,
        detections_json_path=parsed_args.detections_json_path,
